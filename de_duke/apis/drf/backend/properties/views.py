...
class PropertyViewSet(
    ...
    def get_queryset(self):
        ...
        return models.Property.objects.filter(
            ...
            & Q(is_active=True)
            & Q(is_deleted=False)
            & Q(verified__isnull=False)
            & (
                Q(verified__is_auto_verified=True)
                | Q(verified__verification_phase="Verified")
            )
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        property_with_similar = PropertyWithSimilar(property, similar_properties)
...
class BookmarkedPropertyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return models.BookmarkedProperty.objects.filter(
            ...
            & Q(property__banned__isnull=True)
            & Q(property__is_active=True)
            & Q(property__is_deleted=False)
            & Q(property__verified__isnull=False)
            & (
                Q(property__verified__is_auto_verified=True)
                | Q(property__verified__verification_phase="Verified")
            )
        )
    ...
    @action(
        methods=["delete"],
        detail=False,
        url_path="by-property-id/(?P<property_id>[^/.]+)",
        url_name="unbookmark",
    )
...
class HostShortletPropertyViewSet(viewsets.ModelViewSet):
    """
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_host() and self.request.user.is_host_verified():
            return models.ShortletProperty.objects.filter(
                listed_by=self.request.user.verified_host_account(),
                is_deleted=False,
            )
    ...
    def perform_destroy(self, instance):
        ...
        if instance.listed_by == self.request.user.verified_host_account():
    ...
    def create(self, request: Request, *args, **kwargs):
        ...
        if not request.user.is_host() or not request.user.is_host_verified():
            return Response(
                {"detail": "You are not a verified host."},
                status=403,
            )
        ...
        return Response(self.get_serializer(property_instance).data, status=201)
    ...
    def update(self, request: Request, *args, **kwargs):
        ...
        # Return the serialized property instance
        return Response(self.get_serializer(property_instance).data, status=200)
...
class HostCommercialPropertyViewSet(viewsets.ModelViewSet):
    """
    ...
    def create(self, request, *args, **kwargs):
        ...
        for lookup, room_data in rooms.items():
            models.CommercialPropertyRoom.objects.create(
                property=property_instance, **room_data
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        for lookup, room_data in rooms.items():
            if (
                room_data["level"]
                and room_data["dimention_width"]
                and room_data["dimention_length"]
            ):
                if room_data.get("id"):
                    # Update existing room
                    models.CommercialPropertyRoom.objects.filter(
                        id=room_data["id"]
                    ).update(property=property_instance, **room_data)
                else:
                    # Create new room
                    if "id" in room_data:
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_host() and self.request.user.is_host_verified():
            return models.PropertyImage.objects.filter(
                property__listed_by=self.request.user.verified_host_account(),
            )
...
class PropertyFieldChoicesView(APIView):
    """
...
class InterestedPropertyViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        responses={
            200: {"type": "object", "properties": {"websocket_url": {"type": "string"}}}
        },
    )
    @action(
        methods=["get"],
        detail=False,
        url_path="websocket-url",
        url_name="websocket-url",
    )
    def websocket_url(self, request):
        ...
        forwarded_host = request.headers.get("x-forwarded-host") or request.META.get(
